...
class BasePage:
	...
	def wait_for_full_page_load(self, timeout: int = 30000) -> None:
		"""
		Wait for complete page load including DOM and network activity.

		This method waits for both:
		1. DOM content to be loaded and parsed (domcontentloaded)
		2. Network activity to settle (networkidle)

		Use this when you need to ensure the page is fully ready before
		performing assertions or interactions.

		Args:
			timeout: Maximum time to wait in milliseconds for each state
		...
		try:
			...
			logger.debug("Page fully loaded (DOM + network)")
		except PlaywrightTimeoutError:
			logger.error("Timeout waiting for full page load")
			raise
